import { BaseDecoder, IBaseDecoderOptions } from "./BaseDecoder"
import { BitBuffer } from "./BitBuffer"
import { Fill, Now } from "./JSMpeg"

interface IMP2Options extends IBaseDecoderOptions {
  onAudioDecode?: MP2['onDecodeCallback']
  audioBufferSize?: number
  streaming?: boolean
}

interface IMP2Allocation { 
  levels: number
  group: number
  bits: number
}
...
export class MP2 extends BaseDecoder {
  left: Float32Array = new Float32Array(1152)
  right: Float32Array = new Float32Array(1152)
  sampleRate: number = 44100

  D: Float32Array = new Float32Array(1024)
  V: Float32Array = new Float32Array(1024)
  U: Float32Array = new Float32Array(32)
  VPos: number = 0

  allocation: [Array<IMP2Allocation>, Array<IMP2Allocation>] = [new Array(32), new Array(32)]
  scaleFactorInfo: [Uint8Array, Uint8Array] = [new Uint8Array(32), new Uint8Array(32)]
  ...
  public decodeFrame(left: Float32Array, right: Float32Array) {
    // Check for valid header: syncword OK, MPEG-Audio Layer 2
    let sync = this.bits.read(11)
    let version = this.bits.read(2)
    let layer = this.bits.read(2)
    let hasCRC = !this.bits.read(1)
    ...
    let privat = this.bits.read(1)
    let mode = this.bits.read(2)
  ...
  public readAllocation(sb: number, tab3: number): IMP2Allocation {
    let tab4 = MP2.QUANT_LUT_STEP_3[tab3][sb]
    let qtab = MP2.QUANT_LUT_STEP4[tab4 & 15][this.bits.read(tab4 >> 4)]
    return qtab ? (MP2.QUANT_TAB[qtab - 1]) : null
  }

  public readSamples(ch: number, sb: number, part: number) {
    let q = this.allocation[ch][sb]
    let sf = this.scaleFactor[ch][sb][part]
    let sample = this.sample[ch][sb]
    let val = 0
  ...
  static MatrixTransform = function (s: number[][], ss: number, d: Float32Array, dp: number) {
    let t01 = s[0][ss] + s[31][ss]
    let t02 = (s[0][ss] - s[31][ss]) * 0.500602998235
    let t03 = s[1][ss] + s[30][ss]
    let t04 = (s[1][ss] - s[30][ss]) * 0.505470959898
    let t05 = s[2][ss] + s[29][ss]
    let t06 = (s[2][ss] - s[29][ss]) * 0.515447309923
    let t07 = s[3][ss] + s[28][ss]
    let t08 = (s[3][ss] - s[28][ss]) * 0.53104259109
    let t09 = s[4][ss] + s[27][ss]
    let t10 = (s[4][ss] - s[27][ss]) * 0.553103896034
    let t11 = s[5][ss] + s[26][ss]
    let t12 = (s[5][ss] - s[26][ss]) * 0.582934968206
    let t13 = s[6][ss] + s[25][ss]
    let t14 = (s[6][ss] - s[25][ss]) * 0.622504123036
    let t15 = s[7][ss] + s[24][ss]
    let t16 = (s[7][ss] - s[24][ss]) * 0.674808341455
    let t17 = s[8][ss] + s[23][ss]
    let t18 = (s[8][ss] - s[23][ss]) * 0.744536271002
    let t19 = s[9][ss] + s[22][ss]
    let t20 = (s[9][ss] - s[22][ss]) * 0.839349645416
    let t21 = s[10][ss] + s[21][ss]
    let t22 = (s[10][ss] - s[21][ss]) * 0.972568237862
    let t23 = s[11][ss] + s[20][ss]
    let t24 = (s[11][ss] - s[20][ss]) * 1.16943993343
    let t25 = s[12][ss] + s[19][ss]
    let t26 = (s[12][ss] - s[19][ss]) * 1.48416461631
    let t27 = s[13][ss] + s[18][ss]
    let t28 = (s[13][ss] - s[18][ss]) * 2.05778100995
    let t29 = s[14][ss] + s[17][ss]
    let t30 = (s[14][ss] - s[17][ss]) * 3.40760841847
    let t31 = s[15][ss] + s[16][ss]
    let t32 = (s[15][ss] - s[16][ss]) * 10.1900081235
  ...
  static QUANT_LUT_STEP_2 = [
    ...
    [MP2.QUANT_TAB_PARAM.C, MP2.QUANT_TAB_PARAM.C, MP2.QUANT_TAB_PARAM.D], // 32 - 48 kbit/sec/ch
    [MP2.QUANT_TAB_PARAM.A, MP2.QUANT_TAB_PARAM.A, MP2.QUANT_TAB_PARAM.A], // 56 - 80 kbit/sec/ch
    [MP2.QUANT_TAB_PARAM.B, MP2.QUANT_TAB_PARAM.A, MP2.QUANT_TAB_PARAM.B]  // 96+	 kbit/sec/ch
  ]
